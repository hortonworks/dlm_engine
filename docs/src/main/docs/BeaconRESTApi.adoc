...
rangerPrincipalName=ranger/_HOST@YOUR-REALM
...
*Note 1:* dfs.namenode.kerberos.principal, hive.server2.authentication.kerberos.principal are custom properties and are required
if kerberos has been integrated with the cluster. If the cluster is secure and ranger is enabled, we also require rangerPrincipalName.
...
*Note 3:* dfs.nameservices, dfs.internal.nameservices, dfs.ha.namenodes.<nameservice>, dfs.namenode.rpc-address.<nameservice>.<namenode1>,
...
* *_fields:_* Entity output fields separated by commas. Valid options are status, tags, clusters, datasets, instances, frequency, executiontype, customProperties, starttime and endtime.
