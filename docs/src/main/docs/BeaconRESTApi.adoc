...
*FS Replication*
    ...
    * source.setSnapshottable : Allow source dataset to be snapshottable (true|false) (optional). Deprecated since DLM-1.1.3 in favour of 'enableSourceSnapshottable'
    * enableSourceSnapshottable : Allow source dataset to be snapshottable and enforces DLM to use snapshot based replication. If set to 'false', enforces DLM to not to use snapshot based replication regardless of dataset marked to be snapshottable. (true|false) (optional)
...
. link:#\_put_api_beacon_cluster_cluster_name[Update cluster]
...
The different completion status are _SUCCEEDED_, _FAILED_, _SUCCEEDEDWITHSKIPPED_, _FAILEDWITHSKIPPED_, _SUSPENDEDFORINTERVENTION_.
...
dfs.internal.nameservices=primary
...
PUT api/beacon/cluster/:cluster-name
------------------------------------
...
*REST Call:* PUT http://localhost:25968/api/beacon/cluster/primaryCluster
...
*This API does a full update to a cluster and hence the properties supplied in the payload will replace all the existing properties of a cluster.

*Response:*

[source, json]
{
   "requestId":"e5cc8230-f356-4566-9b65-536abdcc8aa3",
   "message":"Update successful (CLUSTER) primaryCluster",
   "status":"SUCCEEDED"
}

*Note 1:* When a cluster is updated, it's existing pairing with other clusters are re-validated. If the re-validation of
 pairing fails, the corresponding cluster pairing status is moved to 'SUSPENDED' state from existing 'PAIRED' state. All
 the policy instances which are running between these two clusters will start skipping, until the paired state is back to
 'PAIRED'.

*Note 2:* Change in paired state from 'PAIRED' to 'SUSPENDED' or from 'SUSPENDED' to 'PAIRED' is automatic and it is
 driven by pairing re-validation which triggers when cluster update takes place.

*Note 3:* Active policy instances detects automatically any change in cluster pairing status.

*Note 4:* 'name', 'version' and 'local' properties of a cluster can not be updated.

*Note 5:* 'local' can be part of payload, but it's existing value can not be changed.


Example of a cluster update with data center in the cluster name.

*REST Call:* PUT http://localhost:25968/api/beacon/cluster/dataCenter$source
...
{
  "status": "SUCCEEDED",
  "message": "Update successful (CLUSTER) dataCenter$source",
  "requestId": "310412574@qtp-1676010932-0"
}
...
* *_fields_* <optional> Entity output fields separated by commas. Valid options are all, peers, peersInfo and tags.
...
*REST Call:* GET http://localhost:25968/api/beacon/cluster/list?fields=all
	...
	"cluster": [
	        {
             "name": "default$c1",
        "version": 1,
        "description": "Prod cluster for hwx",
        "fsEndpoint": "hdfs://test-dlm-25-a-1.c.pramod-thangali.internal:8020",
        "hsEndpoint": "jdbc:hive2://test-dlm-25-a-1.c.pramod-thangali.internal:2181,jaimin-dlm-25-a-2.c.pramod-thangali.internal:2181,jaimin-dlm-25-a-3.c.pramod-thangali.internal:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2",
        "beaconEndpoint": "http://test-dlm-25-a-1.c.pramod-thangali.internal:25968",
        "customProperties": {
            "hive.server2.authentication": "NONE",
            "hive.warehouse.subdir.inherit.perms": "true",
            "cloudDataLake": "false",
            "hive.metastore.warehouse.dir": "/apps/hive/warehouse",
            "hive.metastore.uris": "thrift://test-dlm-25-a-2.c.pramod-thangali.internal:9083",
...
*Note:* statusMessage: this field will contain a message stating the reason for the current state, when the 'pairStatus' is set to 'SUSPENDED'
...
*Description:* Submit and schedule a replication policy

*Parameters:* _:policy-name_ Name of the replication policy

*QueryParameters:* ?suppressWarnings=true/false.Set to true if user wants to ignore the
warning shown during the validation api.By default it is false.
...
If there are no validation errors or warnings.
...
{
    "status": "FAILED",
    "message": "Comma separated validation errors",
    "entityId": null
}

If there are warnings and suppressWarnings is not set to true.

Response code: 400 BAD_REQUEST
[source, json]
{
    "status": "FAILED",
    "message": "comma separated warnings",
    "entityId": null
}

If there are warnings and suppressWarnings is set to true.
...
PUT api/beacon/policy/:policy-name
---------------------------------

*Description:* Update a replication policy

*Parameters:* _:policy-name_ Name of the replication policy

*REST Call:* PUT http://localhost:25968/api/beacon/policy/hivePolicy[__http://localhost:25968/api/beacon/policy/hivePolicy__]

[source, properties]
description=hive daily policy
tde.sameKey=true
frequencyInSec=3600
starTime="yyyy-MM-dd’T’HH:mm:ss" (only valid if policy instance has not run yet)
endTime="yyyy-MM-dd’T’HH:mm:ss"
distcpMapBandwidth=25
distcpMaxMaps=10
queueName=test
plugins=RANGER
enableSnapshotBasedReplication=true

* Payload data can be subset of any of the above properties
...
{
  "status": "SUCCEEDED",
  "message": "Policy [hdfspolicy] update successful",
  "requestId": "1917442783@qtp-1933129092-1"
}

*Note 1:* When 'enableSnapshotBasedReplication' property is updated from 'true' to 'false', all the existing snapshots for the policy are deleted.

POST api/beacon/policy/dryrun/:policy-name
------------------------------------------

*Description:* Perform a dry run on a replication policy
...
If there are any warnings but no validation error.

Response code: 412 PRECONDITION_FAILED.
[source, json]
{
...
*Note:* Different status values for a policy instance: _RUNNING_, _FAILED_, _SUCCESS_, _KILLED_, _SKIPPED_, _FAILED_ADMIN_.
...
*Note:* Different status values for a policy: _SUBMITTED_, _RUNNING_, _SUSPENDED_, _SUCCEEDED_, _FAILED_, _SUCCEEDEDWITHSKIPPED_, _FAILEDWITHSKIPPED_, _SUSPENDEDFORINTERVENTION_.
...
* *_filter_:* Filter on the final component of this path.
...
[source, json]
{
    ...
    "policy_edit": true,
    "clusterUpdateSupported": true,
    "cloudHosted": false,
    "wasbReplicationSupported": true,
    "enableSnapshotBasedReplication": true
...
Supported configurations for WASB:

For WASB access key based.

[source, properties]
name=cloud-cred
provider=WASB
wasb.account.name=<wasb account name>
wasb.access.key=AKIAJ2B5WGGW2QZKJQQA
authtype=WASB_ACCESSKEY

For WASB SAS token based.
[source, properties]
name=cloud-cred-wasb-sas
provider=WASB
wasb.account.name=<wasb account name>
wasb.container.name=<wasb container name>
wasb.sas.token=jYe/fQ6wOi9ebIr5FM7emBHafx3JMzNgjxGmpZMD
authtype=WASB_SAS_TOKEN

Supported configurations for GCS:
...
GET api/beacon/admin/metrics
----------------------------

*Description:* Get Beacon JVM (threads, memory, GC) information and API(s) performance.

*REST Call:* GET http://localhost:25968/api/beacon/admin/metrics
...
[source, json]
{
    ...
    "gauges": {
        ...
        "api.beacon.cluster.submit": {
            "count": 1,
            "max": 1.2877286680000002,
            "mean": 1.2877286680000002,
            "min": 1.2877286680000002,
            "p50": 1.2877286680000002,
            "p75": 1.2877286680000002,
            "p95": 1.2877286680000002,
            "p98": 1.2877286680000002,
            "p99": 1.2877286680000002,
            "p999": 1.2877286680000002,
            "stddev": 0,
            "m15_rate": 0.0011080303990206543,
            "m1_rate": 0.015991117074135343,
            "m5_rate": 0.0033057092356765017,
            "mean_rate": 0.06691466261302709,
...
GET api/beacon/admin/threaddump
--------------------------------

*Description:* Get Beacon thread dump.

*REST Call:* GET http://localhost:25968/api/beacon/admin/threaddump

*Response:*
[source, text]
...
"Attach Listener" id=13 state=RUNNABLE

"QuartzScheduler_Worker-1" id=16 state=TIMED_WAITING
    - waiting on <0x67d9aba6> (a java.lang.Object)
    - locked <0x67d9aba6> (a java.lang.Object)
