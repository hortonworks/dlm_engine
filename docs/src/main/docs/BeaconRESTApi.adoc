...
*FS Replication*
    ...
    * cloud.encryptionAlgorithm : Encryption algorithm with which data has to encrypted on the cloud object store.
    (AES256|SSE-KMS) (optional)
    * cloud.encryptionKey : Encryption key for SSE-KMS encryption algorithm on AWS. (optional)
...
retryAttempts=1
retryDelay=5
cloud.encryptionAlgorithm=SSE-KMS
...
*Note 2:* cloud.encryptionAlgorithm is provided when data has to encrypted on cloud object store. cloud.encryptionKey
 is provided when the encryptionAlgorithm is SSE-KMS
